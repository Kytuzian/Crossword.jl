...
type CrosswordAnswer
    ...
    choices::Array{ASCIIString, 1}
    
    positions::Array{(Int64, Int64), 1}
    
    crosses::Array{(CrosswordAnswer, Int64, Int64), 1}
end

type CrosswordPuzzle
    across::Array{CrosswordAnswer, 1}
    down::Array{CrosswordAnswer, 1}
end
...
function get_all_matches(len::Int64, tests::Array{(Int64, Char), 1}, words::Array{ASCIIString, 1})
    result = ASCIIString[]
    fails = Int64[]
...
function get_all_words()
    ...
    words = ASCIIString[i[1:end-2] for i = readlines(f)]
...
function get_all_choices(a::Array{CrosswordAnswer}, words::Array{ASCIIString, 1})
    for i = a
        if length(i.word) == 0
...
function is_valid(a::Array{CrosswordAnswer, 1})
    for i = a
        if length(i.choices) == 0
            return false
        end
...
function try_combinations(a::Array{CrosswordAnswer, 1}, words::Array{ASCIIString, 1}, tab_level, is_outer = false)
    if !is_valid(a)
    ...
    for (index, choice) = enumerate(a[1].choices)
        ...
        try
            ...
            elseif typeof(e) == (ASCIIString, (Char, Int64))
                if e[1] == a[1].word
                    a[1].word = ""
...
function show_puzzle_grid(grid::Array{Char, 2})
...
function crossword_puzzle(img::ASCIIString, squares)
    read_im = imread(img)
...
function create_answers(a::Array{Int64, 2})
        ...
        for y = 1:size(a)[2]
            if a[x,y] == 1
                ...
                if right && !left
                    ...
                    push!(across_list, CrosswordAnswer("", length(line), ASCIIString[], line, []))
                end
                ...
                if down && !up
                    ...
                    push!(down_list, CrosswordAnswer("", length(line), ASCIIString[], line, []))
                end
            end 
...
function show_grid(grid::Array{Int64, 2})
    for row = 1:size(grid)[1]
        for column = 1:size(grid)[1]
...
black = SimpleColor(float32(0.0), float32(0.0), float32(0.0))
white = SimpleColor(float32(1.0), float32(1.0), float32(1.0))